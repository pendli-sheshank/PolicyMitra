...
quality bar end-to-end against a real embedded SQLite database."""
...
def test_all_three_insurers_ingested(conn):
    with conn.cursor() as cur:
        cur.execute("SELECT insurer, count(*) FROM kb_documents GROUP BY insurer")
...
def test_cataract_row_retrieves_as_one_coherent_chunk_per_insurer(conn):
    with conn.cursor() as cur:
        cur.execute("""
            SELECT text_content FROM kb_chunks
