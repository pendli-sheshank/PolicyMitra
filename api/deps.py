...
def touch_session(conn: psycopg.Connection, session_id: UUID | None, mode: str = "consumer") -> UUID:
    """Lazily expires stale sessions on every touch (docs/architecture.md #8 —
    no pg_cron dependency assumed), then validates or creates a session."""
    with conn.cursor() as cur:
        cur.execute("SELECT mem.expire_sessions()")
